/**
 * POST /api/providers/me/stripe/dashboard
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (user.role !== 'provider') {
      return NextResponse.json({ error: 'Only providers can access dashboard' }, { status: 403 });
    }
    ...
    // 4. Create dashboard login link
    try {
      const dashboardUrl = await createDashboardLink(provider.payout_account_id);
